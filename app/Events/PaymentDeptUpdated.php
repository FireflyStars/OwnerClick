...
class PaymentDeptUpdated
{
    ...
    public function __construct(PaymentDept $paymentDept)
    {
        ...
        if ($paymentDept->wasChanged('active')) {
            if ($paymentDept->active === PaymentDept::PAYMENT_ACTIVE_FALSE) {
                new PaymentDeptDeactivated($paymentDept);
            } else {
                new PaymentDeptActivated($paymentDept);
            }
            die;
        }
        if ($paymentDept->wasChanged('status_id')) {
            return;
        }
...
//        Auth::user()->notify(new PaymentDeptUpdatedNotification($this->paymentDept));
    ...
    static function get($data)
    {
        ...
        $eventMessage->title = __('events.payment_dept_updated_title',['propertyName'=>@$data['propertyName'],'unitName'=>@$data['unitName']]);
        $eventMessage->message = __('events.payment_dept_updated_message',['due_date'=>$data['due_date'],'amount'=>$data['amount'],'currency'=>$data['currency'],'payment_type_id'=>Payment::getPaymentTypes()[$data['payment_type_id']]['name']]);
