...
//        Auth::user()->notify(new PaymentDeptUpdatedNotification($this->paymentDept));
    ...
    static function get($data){
        ...
        $eventMessage->title = __('events.payment_dept_deactivated_title',['propertyName'=>@$data['propertyName'],'unitName'=>@$data['unitName']]);
        $eventMessage->message = __('events.payment_dept_deactivated_message',['due_date'=>$data['due_date'],'amount'=>$data['amount'],'currency'=>$data['currency'],'payment_type_id'=>Payment::getPaymentTypes()[$data['payment_type_id']]['name']]);
