@extends('layouts.login', ['class' => 'off-canvas-sidebar', 'activePage' => 'register', 'title' => __('auth.new_account_title')])
            ...
            <div class="d-table-cell">
                <div class="container">
                        ...
                        <form method="POST" action="{{ route('register') }}">
                            ...
                            <div class="form-group">
                                <input type="text" name="name" class="form-control"
                                       placeholder="{{ __('input_name') }}..." value="{{ old('name') }}" required>
                                @if ($errors->has('name'))
                            ...
                            <div class="form-group">
                                <input type="email" name="email" class="form-control"
                                       placeholder="{{ __('input_email') }}..." value="{{ old('email') }}" required>
                                @if ($errors->has('email'))
                            ...
                            <div class="form-group">
                                <input type="password" name="password" id="password" class="form-control"
                                       placeholder="{{ __('input_password') }}..." required>
                            ...
                            <div class="form-group">
                                <input type="password" name="password_confirmation" id="password_confirmation"
                                       class="form-control" placeholder="{{ __('input_confirm_password') }}..."
                                       required>
                  ...
                  <span class="check"></span>
                                    ...
                                    <a class="text-info" href="{{route('contracts.user-agreement')}}"
                                       rel="modal:open">{{ __('auth.user_agreement') }}</a> {{ __('and') }} <a
                                        href="{{route('contracts.privacy-policy')}}" rel="modal:open"
                                        class="text-info">{{ __('auth.privacy_policy') }}</a> {{ __('auth.I_agree') }}
                                </label>
                            </div>


                            <button type="submit">{{ __('input_create_account') }}</button>

                        </form>
                        <div class="sign-in-with-button">
                            <a href="{{ route('social.redirect','facebook') }}"
                               class="btn btn-just-icon btn-link btn-white" title="{{__('auth.login_with_facebook')}}">
                                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                    ...
                                    <path
                                        ...
                                        opacity=".07"></path>
                                    <path fill="#fff"
                                          d="M33.365,16H36v-3.754c-0.492-0.064-1.531-0.203-3.495-0.203c-4.101,0-6.505,2.08-6.505,6.819V22h-4v4	h4v11h5V26h3.938l0.618-4H31v-2.465C31,17.661,31.612,16,33.365,16z"></path>
                                </svg>
                                {{__('auth.login_with_facebook')}}
                            </a>
                            <a href="{{ route('social.redirect','google') }}"
                               class="btn btn-just-icon btn-link btn-white" title="{{__('auth.login_with_google')}}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18"
                                     aria-hidden="true"><title>Google</title>
                            ...
                            <a target="_top" href="{{ route('social.redirect','linkedin') }}"
                               class="btn btn-just-icon btn-link btn-white" title="{{__('auth.login_with_linkedin')}}">
                                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                     width="24" height="24"
                                     viewBox="0 0 48 48"
                                     style=" fill:#000000;">
                        ...
                        <div class="row align-items-center">
                            <div class="col-lg-6 col-md-6 col-sm-6 dont-account">
                                @if (Route::has('password.request'))
                                    <a href="{{ route('password.request') }}">
                                        <small>{{ __('forgot_password') }}</small>
                                    </a>
                                @endif
                            </div>
                            <div class="col-lg-6 col-md-6 col-sm-6 dont-account">
                                @if (Route::has('login'))
                                    <a href="{{ route('login') }}" class="float-end">
